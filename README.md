...
## Netlify + SendGrid (Email Integration)

This build is wired for **Netlify Email Integration** using a template at:
- `emails/results/index.html`

It sends via a Netlify Function at:
- `/.netlify/functions/send-results`

### Netlify setup
1) Enable Emails extension for the site and select **SendGrid** provider.
2) Add environment variables (Site settings → Environment variables):
   ...
   - `NETLIFY_EMAILS_SECRET=<long random string>`
3) In `netlify/functions/send-results.js`, set `from` to a **verified sender** in SendGrid.
4) Redeploy.

No API keys are stored in browser JS.
