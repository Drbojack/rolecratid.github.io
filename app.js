...
// Render primary role image (diagnostic + normalized)
const visualEl = document.querySelector('.results-visual');
console.log('[RCID] primaryRoles:', primaryRoles);
if (visualEl && primaryRoles.length){
  const raw = primaryRoles[0];
  ...
  const imgPath = ROLE_IMAGES[normalizedRole];
  console.log('[RCID] normalizedRole:', normalizedRole, 'imgPath:', imgPath);

  if (imgPath){
    visualEl.innerHTML = '';
    const img = document.createElement('img');
    img.src = imgPath;
    img.alt = normalizedRole + ' role icon';
    img.style.width = '100%';
    img.style.maxWidth = '260px';
    img.onload = () => console.log('[RCID] image loaded:', img.src);
    img.onerror = () => console.error('[RCID] image failed:', img.src);
    visualEl.appendChild(img);
  } else {
    console.warn('[RCID] No image mapping for role:', normalizedRole);
  }
}
